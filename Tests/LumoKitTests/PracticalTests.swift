...
func testMixedUnicodeAndCode() throws {
    ...
    if !codeChunks.isEmpty {
        let firstCodeChunk = codeChunks[0].text
        let containsUnicode = firstCodeChunk.contains("世界")
        let containsGreet = firstCodeChunk.contains("greet")
        #expect(containsUnicode || containsGreet, "Code should preserve content")
    }
}
